...
Backend API MUST manage multiple API keys for external services (OpenRouter) and implement automatic fallback when one key fails. Dashboard frontend calls backend proxy endpoints, NOT direct external APIs.
...
### Backend Setup
1. Store API keys in AWS Secrets Manager or environment variables
...
### Frontend Setup
1. Call backend proxy endpoints (`/api/ocr/process`, `/api/users/list`, etc.)
