...
All API calls to external services (OpenRouter, backend Lambda) MUST implement retry logic with exponential backoff for transient errors (503, 429, network timeouts).
...
**Problem**: External APIs (OpenRouter, AWS Lambda) can experience transient failures (503 service unavailable, 429 rate limit). Failing immediately wastes user time and creates poor UX.

**Solution**: Exponential backoff gives the service time to recover while avoiding thundering herd. 3 retries with 1s, 2s, 4s delays = 7s total wait, acceptable for admin dashboard.
...
- **User-initiated actions**: Show retry progress in UI, allow cancel
